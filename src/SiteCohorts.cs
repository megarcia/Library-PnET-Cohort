...
namespace Landis.Library.PnETCohorts
{
    public class SiteCohorts : Landis.Library.UniversalCohorts.SiteCohorts, ISiteCohorts
    {
        ...
        private static float ComputeMaxSnowMelt(float Tavg, float DaySpan)
        {
        ...
        public bool Grow(List<IEcoregionPnETVariables> data, bool AllowMortality = true, bool outputCohortData = true)
        {
            ...
            for (int m = 0; m < data.Count(); m++)
            {
                ...
                if (data[m].Tavg <= 0)
                    ...
                    daysOfWinter = 0;
                float Psno_kg_m3 = Snow.CalcDensity(daysOfWinter);
                float snowDepth = Snow.CalcDepth(Psno_kg_m3, snowpack);
                ...
                if (soilIceDepth)
                {
                    ...
                    if (propRootAboveFrost < 1) // If part of the rooting zone is frozen
                    {
                        if (propNewFrozen > 0)  // freezing
                        {
                            ...
                            bool successdepth = hydrology.SetFrozenDepth(Ecoregion.RootingDepth * propRootBelowFrost); // Volume of rooting soil that is frozen                                                                                                                       // Water is a volumetric value (mm/m) so frozen water does not need to be removed, as the concentration stays the same
                        }
                    }
                ...
                float RET = hydrology.Calculate_RET_Hamon(data[m].Tavg, data[m].Daylength); //mm/day
                ...
                float groundPET = hydrology.Calculate_PotentialGroundET_LAI(CanopyLAI.Sum(), data[m].Tavg, data[m].Daylength, data[m].DaySpan, ((Parameter<float>)Names.GetParameter("ETExtCoeff")).Value);
                ...
                else // When no cohorts are present
                {
                    if (MeltInWater > 0)
                    {
                        ...
                        if (success == false)
                            throw new System.Exception("Error adding water, MeltInWaterr = " + MeltInWater + "; water = " + hydrology.Water + "; meltrunoff = " + meltrunoff + "; ecoregion = " + Ecoregion.Name + "; site = " + Site.Location);
