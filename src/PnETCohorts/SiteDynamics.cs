...
namespace Landis.Library.PnETCohorts
{
    public class SiteDynamics
    {
        ...
        public static void siteSuccession(Landis.Library.PnETCohorts.SiteCohorts siteCohorts)
        {
            int RDflag;
        ...
        public static void NaturalMortality(Landis.Library.PnETCohorts.SiteCohorts siteCohorts, int StartAge)
        {
            ...
            if (StartAge == 0)
            {
                ...
                foreach (Landis.Library.PnETCohorts.ICohort cohort in siteCohorts.AllCohorts)
                {
                ...
                //====================================================================================================
                foreach (Landis.Library.PnETCohorts.ISpeciesCohorts speciesCohorts in siteCohorts)
                {
                    ISpeciesDensity speciesDensity = SpeciesParameters.SpeciesDensity.AllSpecies[speciesCohorts.Species.Index];
                    foreach (Landis.Library.PnETCohorts.Cohort cohort in speciesCohorts)
                    {
                        if (speciesDensity.SpType >= 0)
                        {
                            double TmpMortality = Landis.Library.PnETCohorts.Cohorts.SuccessionTimeStep / 10 / (1.0 + Math.Exp(3.25309 - 0.00072647 * tmpDQ + 0.01668809 * cohort.Diameter / 2.54));
            ...
            else //kill youngest tree
            {
                ...
                foreach (Landis.Library.PnETCohorts.ISpeciesCohorts speciesCohorts in siteCohorts)
                {
                    ...
                    foreach (Landis.Library.PnETCohorts.Cohort cohort in speciesCohorts)
                    {
                        if (speciesDensity.SpType >= 0)
                        {
                            if (cohort.Age <= (Landis.Library.PnETCohorts.Cohorts.SuccessionTimeStep * StartAge))
                            {
                                double TmpMortality = Landis.Library.PnETCohorts.Cohorts.SuccessionTimeStep / 10 / (1.0 + Math.Exp(3.25309 - 0.00072647 * tmpDQ + 0.01668809 * cohort.Diameter / 2.54));
                                TmpMortality = (1.0f < TmpMortality ? 1.0f : TmpMortality);
        ...
        public static void Selfthinning(Landis.Library.PnETCohorts.SiteCohorts siteCohorts)
        {
            ...
            //SortedList<double, Landis.Library.PnETCohorts.ICohort> cohortMortality = new SortedList<double, Library.DensityCohorts.ICohort>();
        ...
        public static float computeMortalityRD(Landis.Library.PnETCohorts.ICohort cohort, int deadTrees)
        {
