...
namespace Landis.Library.PnETCohorts
{
    public class Snow
    {
        ...
        {
            float depth = Constants.DensityWater * Snowpack / DensitySnow_kg_m3 / 1000.0f;
            return depth;
        }
        ...
        {
            float heatcapacity = Constants.HeatCapacitySnow_Jperkg * DensitySnow_kg_m3 / 1000.0f;
            return heatcapacity;
        }
        ...
        {
            float conductivity = (float)(Constants.ThermalConductivityAir_Watts + ((0.0000775f * DensitySnow_kg_m3) + (0.000001105f * Math.Pow(DensitySnow_kg_m3, 2))) * (Constants.ThermalConductivityIce_Watts - Constants.ThermalConductivityAir_Watts)) * 3.6f * 24.0f;
            return conductivity;
        }
        ...
        /// ----------
        /// MG20260915: the CLM reference is no longer available and the reference
        /// to Jordan (1991) is invalid -- this is not eq. 85 in the Jordan (1991)
        ...
        /// <summary>
        /// Snowmelt rate can range between 1.6 to 6.0 mm/degree day, and default
        /// should be 2.74 according to NRCS Part 630 Hydrology National Engineering
        /// Handbook (Chapter 11: Snowmelt)
        /// </summary>
        ...
        {
            float maxsnowmelt = 2.74 * (float)Math.Max(0.0, Tavg) * DaySpan;
            return maxsnowmelt;
        }
        ...
        {
            float snowfrac = (float)Math.Max(0.0, Math.Min(1.0, (Tavg - 2.0) / -7.0));
            return snowfrac;
        }
        ...
        public static float CalcNewSnowDepth(float Tavg, float Prec, float SublimationFrac)
        {
            ...
            float NewSnowDepth = NewSnow * (1.0 - SublimationFrac); // (mm) Account for sublimation here
            if (NewSnowDepth < 0.0 || NewSnowDepth > Prec)
