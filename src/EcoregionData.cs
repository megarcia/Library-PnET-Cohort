...
namespace Landis.Library.PnETCohorts
{
    ...
    public class EcoregionData : IEcoregionPnET
    {
            ...
            lock (Globals.ecoregionDataThreadLock)
            {
                while (end.Ticks > date.Ticks)
                {
                    if (!all_values[ecoregion].ContainsKey(date))
                    {
                        ...
                        {
                            int newYear;
                            if (ClimateInput.IsFutureClimate(date))
                            {
                                newYear = ClimateInput.ConvertYearToFutureClimateYear(date);
                                ClimateRegionData.AnnualClimate[ecoregion] = Climate.Climate.FutureEcoregionYearClimate[ecoregion.Index][newYear];
                            }
                            else
                            {
                                newYear = ClimateInput.ConvertYearToSpinUpClimateYear(date);
                                ClimateRegionData.AnnualClimate[ecoregion] = Climate.Climate.SpinupEcoregionYearClimate[ecoregion.Index][newYear];
                            }
                            oldYear = date.Year;
                        }
        ...
        public static void Initialize()
        {
            ...
            foreach (IEcoregionPnET ecoregion in AllEcoregions.Values)
        ...
        {
            parameter = null;
            if ((label == null) || (Names.parameters.ContainsKey(label) == false))
                return false;
            else
            {
