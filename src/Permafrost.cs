...
namespace Landis.Library.PnETCohorts
{
    public class Permafrost
    {
        ...
        public static SortedList<float, float> CalculateMonthlySoilTemps(SortedList<float, float> depthTempDict,IEcoregionPnET Ecoregion, int daysOfWinter, float snowPack, IHydrology hydrology, float lastTempBelowSnow)
        {
            ...
            float ThermalConductivitySoil_Watts = PressureHeadSaxton_Rawls.GetLambda_s(Ecoregion.SoilType);
            float lambda_theta = (Fs * (1.0F - porosity) * ThermalConductivitySoil_Watts + Fa * (porosity - waterContent) * Constants.ThermalConductivityAir_Watts + waterContent * Constants.ThermalConductivityWater_Watts) / (Fs * (1.0F - porosity) + Fa * (porosity - waterContent) + waterContent); //soil thermal conductivity (kJ/m/d/K)
            ...
            float d = (float)Math.Sqrt(Constants.omega / (2.0F * Dmonth));
        ...
        public static float CalculateSnowDamping(float Psno_kg_m3)
        {
            // from CLM model - https://escomp.github.io/ctsm-docs/doc/build/html/tech_note/Soil_Snow_Temperatures/CLM50_Tech_Note_Soil_Snow_Temperatures.html#soil-and-snow-thermal-properties
            // Eq. 85 - Jordan (1991)
            float lambda_Snow = (float)(Constants.ThermalConductivityAir_Watts + ((0.0000775f * Psno_kg_m3) + (0.000001105f * Math.Pow(Psno_kg_m3, 2))) * (Constants.ThermalConductivityIce_Watts - Constants.ThermalConductivityAir_Watts)) * 3.6f * 24f; //(kJ/m/d/K) includes unit conversion from W to kJ
